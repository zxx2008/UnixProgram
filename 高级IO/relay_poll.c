/*需求：有左右两个设备，第一个任务为读左设备，写右设备，第二个任务读右设备，写左设备。使用poll函数进行重构，使得在有限状态机在read阶段不会忙等*/
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>
...
static void relay(int fd1, int fd2) {
    ...
    while (fsm12.stat != STATE_T || fsm21.stat != STATE_T) {
        ...
        //判断推进状态机的情况
        if (pfd[0].revents & POLLIN || pfd[1].revents & POLLOUT ||fsm12.stat > STATE_AUTO) {
            fsm_driver(&fsm12);
        }

        if (pfd[0].revents & POLLOUT || pfd[1].revents & POLLIN || fsm21.stat > STATE_AUTO) {
            fsm_driver(&fsm21);
        }
        //fsm_driver(&fsm12);
        //fsm_driver(&fsm21);
